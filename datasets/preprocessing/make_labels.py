...
label_directory = "/home/allparel/Allparel-ML/datasets/labels/"
...
def update_db_records(records):
    ...
    db = client.allparel
    collection = db.clothes3
    bulk = collection.initialize_unordered_bulk_op()

    r_all = []
    for rec in records:
        try:
            bulk.find({'image_file':rec.image_filename}).upsert().update({ '$set': rec.dict()})

        except Exception as e:
            print(e)
            print(rec)
    print("Finished writing, about the bulk execute...") 
    try:
        bulk.execute()
    except Exception as e:
        print(e)
...
def read_db_records():
    ...
    for rec in db_records:
        ...
        if 'product_url' not in rec:
            print(rec)
        product_url = rec['product_url']
        raw_category = rec['raw_category']
        category = rec['category']
        pos = rec['positive_tags']
        neg = rec['negative_tags']
        
        records.append(record.Record(image_file, title, description, product_url, raw_category, category, pos, neg))
    return records
...
def db_allparel():
    ...
    count = 0
    for rec in db_records:
        if "allparel_tags" in rec and len(rec["allparel_tags"]) > 0:
            count = count + 1
    print(len(count))
